...
CANONICAL_CHROMS = tuple(
    [f"chr{chrom}" for chrom in range(1, 20)] + ["chrX", "chrY"]
)
...
def parse_model_alleles(value: Any) -> dict[str, str]:
    ...
    return {
        symbol: ";".join(values)
        for symbol, values in alleles.items()
    }
...
def write_parquet(
    ...
    arrays = [
        pa.array(columns[field], type=schema[field])
        for field in fieldnames
    ]
